...
def show_predictions(models):
    ...
    try:
        ...
        m2_features = load_pkl("model2_features.pkl")   # ['cycle_day','normalized_cycle_day','estrogen','pdg','hr_mean']
    ...
    # ------------------------------------------------------------------
    # 📌 MODEL 2 — PHASE PREDICTION (user-friendly input)
    # ------------------------------------------------------------------
    with tab1:
        st.header("📌 Phase Prediction")
        st.markdown("Provide simple cycle + hormone information to predict: **Menstrual, Follicular, Fertile, or Luteal Phase**.")
        
        # Only user-facing inputs
        ...
        if st.button("Predict Phase"):
            ...
            X = pd.DataFrame([input_dict])[m2_features]
    ...
    # ------------------------------------------------------------------
    # 📌 MODEL 3 — CYCLE LENGTH PREDICTION (unchanged)
    # ------------------------------------------------------------------
    with tab2:
